...
class Mesa18(AutotoolsPackage):
    ...
    def osmesa_libs(self):
        return find_libraries("libOSMesa", root=self.spec.prefix, shared=True, recursive=True)

    @property
    def glx_libs(self):
        return find_libraries("libGL", root=self.spec.prefix, shared=True, recursive=True)
